...
Compared to the general-form QAP as in~\cite{harvind2005symbol}, each KB-form
QAP is defined with two $Q$-by-$Q$ matrices instead of one $Q^4$ 4-dimensional
matrix, and only one of the two matrices needs to be updated. The overall
computational complexity is greatly reduced, therefore
much larger constellation can be handled in this case
than~\cite{harvind2005symbol}. Since the MoDiv design depends only on statstical
CSI, the QAP problems are solved off-line with an efficient robust taboo search
algorithm~\cite{taillard1991robust}. We note that other numerical
approaches to the KB-form QAP are also available, including simulated
anneling (SA)~\cite{}, and so forth.
